...
class TagFeather extends Builder
{
	...
	{
		if($this->builder->has_build){return $this->cache_dir.$this->dest;}
		
		$this->builder->has_build=$this->hookmanager->call_parsehooksbytype('modifier',$this->has_build);
		if($this->builder->is_build_error){return "";}
	...
	public function getTemplateFile($source,$ignore_in_cache=false,$is_forcebuild=false)
	{
		...
		$this->source=$source;
		if($is_forcebuild)$this->forcebuild();
	...
	*/
	...
	{
		if($GLOBALS['TF_IN_CACHE']){return;}
		if($structfile){$this->add_struct($structfile);}

...
class Hooks
{
	...
	{
		if($tf->is_forcebuild){return false;}
		...
		foreach($tf->struct_files as $filename){
			...
			if($theTime>$maxtime)$maxtime=$theTime;
		}
		$sourceTime=$maxtime;
	...
	public static function prebuild_struct($data,$tf,$hooktype)
	{
		...
		if(!$tf->struct_files){ return $data;}
	...
	{
		if(!is_array($tf->runtime['ssidel']) )return $data;
	...
	{
		if(!$tf->is_outputphp)return $data;
	...
	{
		if($tf->runtime['doctype_writed'])return '';
	...
	{
		if(!$tf->is_outputphp)return $data;
		if(0===strncmp($data,"<\x3fphp",strlen("<\x3fphp")))return $data;
		if(0===strncmp($data,"<\x3fxml",strlen("<\x3fxml")))return $data;
	...
	public static function tagend_tf_final($attrs,$tf,$hooktype)
	{
		...
		if(array_key_exists('tf:tagname',$attrs)){$attrs["\ntagname"]=$attrs['tf:tagname'];}
		if(TF_Helper::GetBool($attrs['tf:notag'])){$attrs["\ntagname"]='';}
		
		if(array_key_exists('tf:text',$attrs)){
			$attrs["\ntext"]=$attrs['tf:text'];
		}
		if(TF_Helper::GetBool($attrs['tf:notext'])){$attrs["\ntext"]='';}
		$attrs["\ntext"]=$attrs['tf:pretext'].$attrs["\ntext"].$attrs['tf:posttext'];
		
		if(array_key_exists('tf:pretag',$attrs)){$attrs["\npretag"]=$attrs['tf:pretag'];}
		if(array_key_exists('tf:posttag',$attrs)){$attrs["\nposttag"]=$attrs['tf:posttag'];}
	...
	public static function tagend_showonce($attrs,$tf,$hooktype)
	{
		...
		if(!$attrs['tf:showonce'])return $attrs;
	...
	{
		if( !TF_Helper::GetBool($attrs['tf:phpheredoc']) )return $attrs;
	...
	{
		if($attrs["\ntagname"]!='meta'){return $attrs;}
		if($tf->runtime['meta_writed'])return TF_Helper::ToBlankAttrs($attrs);
	...
	{
		if(!array_key_exists('tf:delattrs',$attrs))return $attrs;
	...
	{
		if(!array_key_exists('tf:over',$attrs)) return $attrs;
	...
	{
		if( !TF_Helper::GetBool($attrs['tf:showstruct']) )return $attrs;
		$l=sizeof($tf->tagStack);
		for($i=$l-1;$i>=0;$i--){
			$tag=&$tf->tagStack[$i];
			if($tag['tf:struct']){
	...
	{
		if(!array_key_exists('tf:struct',$attrs))return $attrs;
		$tf->parser->current_line=$tf->parser->current_line-$attrs['tf:struct_lines'];
		if($tf->in_struct==sizeof($tf->tagStack)){$tf->in_struct=0;}
		if($tf->parser->current_line<1)$tf->parser->current_line=1;
		return $attrs;
	}
	/************************/
	public static function tagend_bindas($attrs,$tf,$hooktype)
	{
		...
		if(!array_key_exists('tf:bindas',$attrs))return $attrs;
	...
	public static function tagend_bindwith($attrs,$tf,$hooktype)
	{
		...
		if(!array_key_exists('tf:bindwith',$attrs) && !TF_Helper::GetBool($attrs['tf:bindwith']))return $attrs;
		$id='';$cmd='';
	...
	{
		//$tf->runtime['bind']=isset($tf->runtime['bind'])?$tf->runtime['bind']:array();
		if(!array_key_exists('tf:bindto',$attrs))return $attrs;
		...
		if( array_key_exists('tf:bindto_mode',$attrs) ){
			...
			if("insert"==$mode){$text=$text.$tf->runtime['bind'][$cmd];}
			if("append"==$mode){$text=$tf->runtime['bind'][$cmd].$text;}
		}
	...
	{
		if( !(array_key_exists('tf:wrap_data',$attrs)) ){return $attrs;}
	...
	{
		if(!array_key_exists('tf:wrap',$attrs)){return $attrs;}
		...
		if(!$header){$header=$headers['pi'];}
		
		$footer=$footers[$attrs['tf:wrap']];
		if(!$footer){$footer=$footers['pi'];}
	...
	{
		if(!array_key_exists('tf:phpincmap',$attrs))return $attrs;
	...
	{
		if( !(array_key_exists('tf:delhead',$attrs))  && !(array_key_exists('tf:delfoot',$attrs)) ){return $attrs;}
		if($tf->in_struct){return $attrs;}
	...
	public static function tagend_headfoot($attrs,$tf,$hooktype)
	{
		...
		if( !(array_key_exists('tf:head',$attrs))  && !(array_key_exists('tf:foot',$attrs)) ){return $attrs;}
		if($attrs['tf:head']){
	...
	{
		if(!array_key_exists('tf:phplang',$attrs))return $attrs;
	...
	{
		if( !array_key_exists('tf:textmap',$attrs) && !TF_Helper::GetBool($attrs['tf:textmap'])){return $attrs;}
		$text=TF_Builder::GetTagText($attrs);
	...
	public static function tagend_attrmap($attrs,$tf,$hooktype)
	{
		...
		if( !flag && !$tf->runtime['attrmap']){return $attrs;}
		if($flag){
			$text=TF_Builder::GetTagText($attrs);
			...
			foreach($tf->runtime['attrmap'] as $keytomap => $array){
				if(!array_key_exists($keytomap,$attrs)){continue;}
	...
	{
		if( !array_key_exists('tf:bindmap',$attrs) && !TF_Helper::GetBool($attrs['tf:bindmap'])){return $attrs;}
		$text=TF_Builder::GetTagText($attrs);
	...
	public static function tagbegin_safe($attrs,$tf,$hooktype)
	{
		if(array_key_exists('tf:safe',$attrs) && TF_Helper::GetBool($attrs['tf:safe']) ){
			...
			if(false!==$pi_pos){ $data=substr($data,0,$pi_pos);}
			$asp_pos=strpos($data,"<\x25");
			if(false!==$asp_pos){ $data=substr($data,0,$asp_pos);}
			if($headdata){ $data=$headdata.$data; }
			$tf->parser->data=$data;
			return $attrs;
		}
	...
	{
		if(!$attrs['tf:struct'])return $attrs;
		...
		if( TF_Helper::GetBool($attrs['tf:struct_loaded']) )return $attrs;
	...
	public static function tagbegin_tochildren($attrs,$tf,$hooktype)
	{
		...
		if( array_key_exists('tf:tochildren',$attrs) ){
			...
			$obj_assign=TF_Helper::AttrsPrepareAssign($attrs,'tf:tochildren','tf:tochildren_showtag',
				'tf:tochildren_showtext','tf:tochildren_onlyserver');
	...
	{
		if(!array_key_exists('tf:bycookie',$attrs)){return $attrs;}
		...
		if("tagend"!=$hooktype){return $attrs;}
	...
	{
		if(!array_key_exists('tf:inserttoparse',$attrs)){return $attrs;}
	...
	{
		if(!array_key_exists('tf:appendtoparse',$attrs)){return $attrs;}
	...
	public static function tagbegin_pure($attrs,$tf,$hooktype)
	{
		...
		if($hooktype!="tagbegin" && $hooktype!="tagend"){return $attrs;}
	...
	public static function ssi_include($attrs,$tf,$hooktype)
	{
		...
		if('include'!=$attrs['#']){ return $attrs;}
		$htmlfile=$attrs['file'];
		if(!isset($tf->runtime['phpincmap']))return $attrs;
		
		if(!array_key_exists($htmlfile,$tf->runtime['phpincmap']))return $attrs;
	...
	{
		if('noparseend'==$attrs['#']){ return array();}
		if('noparsebegin'!=$attrs['#']){ return $attrs;}
		$data=$tf->parser->data;
		$pos=strpos($data,"<!--#noparseend");
		if(FALSE===$pos)return $attrs;
	...
	{
		if('appendtoparse'!=$attrs['#']){ return $attrs;}
	...
	{
		if('appendtotextend'==$attrs['#']){ return array();}
		if('appendtotextbegin'!=$attrs['#']){ return $attrs;}
		$data=$tf->parser->data;
	...
	{
		if('delbegin'!=$attrs['#']){ return $attrs;}
		$id=$attrs['id'];
		if(!$id){
			$id=uniqid();
	...
	{
		if('tagbegin'!=$attrs['#']){ return $attrs;}
		unset($attrs['#']);
	...
	public static function all_quick_function($attrs,$tf,$hooktype)
	{
		...
		if(function_exists($func)){	$attrs=$func($param,$tf,$hooktype);}
		
		switch($hooktype){
		case 'ssi':
			$param=$attrs;
			$func="tf_".$hooktype."_".$attrs['#'];
			if(function_exists($func)){ return $func($param,$tf,$hoktype);	}
		break;
		case 'tagbegin':
		case 'tagend':
			if(!is_array($attrs)){
